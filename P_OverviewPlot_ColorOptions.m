...
% sampleP = ["LibBval_Pop_S2d5" "LibBval_Pop_S2d12" "LibBvalwS2d12_30"]
...
%nameinP = ["PopBVALevoDM d5" "PopBVALevoDM d12" "BVALevoDM"];
nameinP = ["PopBVALevoCM d3" "PopBVALevoCM d5" "BVALevoCM"];


% plot the color code either per length or variant frequency
plotMode = "frequency";    % length is default plotMode = []; but you can switch to "frequency"
...
if sum(clu~=0)>0
    ...
    for i=1:numel(clu)
        if clu(i)~=0
            ...
            VarFreq = [VarFreq;Cluster(pos).ClusterFrequency(:,1)];
        end
    end
    ...
    if plotMode == "frequency"
        clrlimit = round(max(VarFreq)*100);
    end
    
    clrmap=(cool(clrlimit));
    clrmap(end+1,:)=[0 0 0];
    ...
    for i = 1:numel(sampleP)
        if clu(i)~=0
            ...
            % for the length variant
            CS = Cluster(idx).C(1,:);
            CE = Cluster(idx).C(2,:);
            CL = Cluster(idx).Cdist;
            % for the frequency variant
            Cf = round(Cluster(idx).ClusterFrequency(:,1)*100);
            ...
            for j = 1:size(CS,2)
                if CS(j)>CE(j)
                    for b = 1:rows
                        ...
                        hold on;
                        if plotMode == "frequency"
                            plot([CS(j) recipsize],[L L],'color',clrmap(Cf(j),:),'linewidth',8)
                            plot([1 CE(j)],[L L],'color',clrmap(Cf(j),:),'linewidth',8)
                        end
                    end
                else
                    for b = 1:rows
                        ...
                        hold on;
                        if plotMode == "frequency"
                            plot([CS(j) CE(j)],[L L],'color',clrmap(Cf(j),:),'linewidth',8)
                        end
                    end
                end
            end
    ...
    if plotMode == "frequency"
        tickLabel = cellstr(string(ticks));
        colorbar('eastoutside','TickLabels',{tickLabel{1:end-1} strcat(tickLabel{end},' %') 'interpreter' 'latex'}, ...
        'Ticks', 0:stepsize:clrlimit);
    else 
...
if sum(denovo~=0)>0
    for i = 1:numel(sampleP)
        ...
        if denovo(i)==1
            deno = [];
            deno = double(Cluster(idx).denovo);
...
if 	SavePlot == "ON"
    print(gcf, '-painters', '-dpng', [savepath + datestr(now, "yyyymmdd") + "_DM_OverviewPlot_" + PlotName]);
else
